...
class getid3_id3v2
{
		...
		if (isset($parsedFrame['datalength']) && ($parsedFrame['datalength'] == 0)) {
			...
			if ($id3v2_majorversion == 2 && strlen($parsedFrame['data']) > $frame_offset) {
				...
				if (strtolower($frame_imagetype) == 'ima') {
					...
					// MIME type instead of 3-char ID3v2.2-format image type  (thanks xbhoffРЁpacbell*net)
					$frame_terminatorpos = strpos($parsedFrame['data'], "\x00", $frame_offset);
	...
	function LookupCurrencyCountry($currencyid) {
		...
		/** This is not a comment!
			...
			VUV	Vanuatu
			WST	Samoa
			XAF	CommunautР№ FinanciРёre Africaine
			XAG	Silver
			XAU	Gold
			XCD	East Caribbean
			XDR	International Monetary Fund
			XPD	Palladium
			XPF	Comptoirs FranР·ais du Pacifique
			XPT	Platinum
			YER	Yemen
			YUM	Yugoslavia
			ZAR	South Africa
	...
	static function TextEncodingNameLookup($encoding) {
		// http://www.id3.org/id3v2.4.0-structure.txt
		// Frames that allow different types of text encoding contains a text encoding description byte. Possible encodings:
	static $TextEncodingNameLookup = array(0=>'cp1251', 1=>'UTF-16', 2=>'UTF-16BE', 3=>'UTF-8', 255=>'UTF-16BE');
