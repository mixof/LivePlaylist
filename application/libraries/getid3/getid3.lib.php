...
class getid3_lib
{
	...
	static function MultiByteCharString2HTML($string, $charset='UTF-8') {
		$HTMLstring = '';
			...
			foreach ($ThisFileInfo['comments'] as $field => $values) {
				foreach ($values as $index => $value) {
					if (is_array($value)) {
						...
						$ThisFileInfo['comments_html'][$field][$index] = str_replace('&#0;', '', getid3_lib::MultiByteCharString2HTML($value, 'cp1251'));
					}
				}
			}
		}
